...
from tests.auth.constants import REGISTER_PATH, VALID_REGISTER_USER, REFRESH_PATH, \
    REFRESH_TOKEN_SCHEMA, LOGOUT_ALL_PATH, VALID_LOGIN_USER, LOGIN_PATH


@pytest.mark.asyncio
async def test_logout_all_success(client: AsyncClient):
    ...
    response = await client.post(LOGOUT_ALL_PATH, json=json)
...
@pytest.mark.asyncio
async def test_logout_all_revoke_token(client: AsyncClient, debug_response):
    """Проверяет отказ при повторном входе после logout-all"""
    ...
    json = REFRESH_TOKEN_SCHEMA.copy()
    json["refresh_token"] = refresh_token
    await client.post(LOGOUT_ALL_PATH, json=json)
...
@pytest.mark.asyncio
async def test_logout_all_other_session(client: AsyncClient):
    """Проверяет отсутствие возможности входа после logout для другой сессии"""
