...
from src.schemas.ratings import RatingAvgValue, RatingShortOut

ratings = APIRouter(prefix="/rating", tags=["Rating of routes"])


@ratings.get("/", response_model=List[RatingShortOut])
...
@ratings.get("/my", response_model=List[RatingShortOut])
...
@ratings.get("/detail", response_model=RatingShortOut)
...
@ratings.get("/{route_id}", response_model=RatingShortOut)
...
@ratings.get("/avg/{route_id}", response_model=RatingAvgValue)
...
@ratings.post("/", response_model=RatingShortOut)
...
@ratings.put("/", response_model=RatingShortOut)
